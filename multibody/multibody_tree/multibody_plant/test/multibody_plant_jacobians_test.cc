...
TEST_F(KukaIiwaModelTests, CalcRelativeFrameAnalyticalJacobian) {
  ...
  plant_->tree().CalcRelativeFrameAnalyticalJacobian(*context_,
  ...
  plant_autodiff_->SetPositions(
      context_autodiff_.get(), q_double.cast<AutoDiffXd>());
  // Set the velocity values in the context using q̇.
  plant_autodiff_->tree().MapQDotToVelocity(
      *context_autodiff_, qdot_autodiff, &v_autodiff);
  ...
  Isometry3<AutoDiffXd> X_WE_autodiff =
      plant_autodiff_->tree().EvalBodyPoseInWorld(
          *context_autodiff_, end_effector_link_autodiff);
  ...
  EXPECT_TRUE(CompareMatrices(
      Jq_WEp, Jq_WEp_autodiff, kTolerance, MatrixCompareType::relative));
}
