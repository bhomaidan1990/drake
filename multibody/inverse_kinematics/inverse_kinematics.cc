...
solvers::Binding<solvers::Constraint> InverseKinematics::AddPositionConstraint(
  ...
  auto constraint = std::make_shared<internal::PositionConstraint>(plant_,
      frameB, p_BQ, frameA, p_AQ_lower, p_AQ_upper, get_mutable_context());
...
InverseKinematics::AddOrientationConstraint(
    ...
    double angle_bound) {
  auto constraint =
      std::make_shared<internal::OrientationConstraint>(plant_, frameAbar,
          R_AbarA, frameBbar, R_BbarB, angle_bound, get_mutable_context());
  return prog_->AddConstraint(constraint, q_);
}
...
InverseKinematics::AddGazeTargetConstraint(
  ...
  auto constraint = std::make_shared<internal::GazeTargetConstraint>(plant_,
      frameA, p_AS, n_A, frameB, p_BT, cone_half_angle, get_mutable_context());
  return prog_->AddConstraint(constraint, q_);
}
...
InverseKinematics::AddAngleBetweenVectorsConstraint(
  ...
  auto constraint =
      std::make_shared<internal::AngleBetweenVectorsConstraint>(plant_, frameA,
          na_A, frameB, nb_B, angle_lower, angle_upper, get_mutable_context());
  return prog_->AddConstraint(constraint, q_);
}
}  // namespace multibody
}  // namespace drake
