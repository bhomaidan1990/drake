...
void PositionConstraint::DoEval(
  ...
  Eigen::MatrixXd Jv_ABq(6, plant_.num_positions());
  plant_.tree().CalcRelativeFrameGeometricJacobian(
      *context_, frameB_, p_BQ_, frameA_, frameA_, &Jv_ABq);
  *y =
      math::initializeAutoDiffGivenGradientMatrix(p_AQ, Jv_ABq.bottomRows<3>());
}
